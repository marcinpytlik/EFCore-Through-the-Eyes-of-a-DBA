...
| SQL without `minId` |  |
| SQL with `?minId=1000` |  |
| First IDs without `minId` |  |
| First IDs with `minId=1000` |  |
| Additional predicate in the same round trip? |  |
...
| Item | Result |
|---|---|
| READ COMMITTED behavior (HTTP) |  |
| READ UNCOMMITTED behavior (HTTP) |  |
| Dirty read observed? |  |
| Value while hold is in flight |  |
...
| Item | Result |
|---|---|
| Deadlock victim (HTTP 409 / session) |  |
| SQL error |  |
